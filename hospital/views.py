...
#for showing signup/login button for pathologist(by sumit)
def pathologistclick_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'pathologist/pathologistclick.html')
...
def afterlogin_view(request):
    ...
    if is_admin(request.user):
        ...
        if accountapproval:
            return redirect('doctor/doctor-dashboard/') #you can also use 'doctor-dashboard' that is name of url
        else:
        ...
        accountapproval = Pathologist.objects.filter(user_id=request.user.id, status=True)
        if accountapproval:
...
def approve_patient_view(request, pk):
    ...
    group_name = f"user_{patient.user.id}"
    async_to_sync(channel_layer.group_send)(
        group_name,
        {
            "type": "send.notification",
            "message": {"text": "✅ Your account has been approved!"}
        }
    )
...
def discharge_patient_view(request,pk):
    ...
    if request.method == 'POST':
        ...
        pDD.save()
        return render(request,'patient/patient_final_bill.html',context=patientDict)  #⬇️
...
import io                          # In-memory stream to hold PDF data
from xhtml2pdf import pisa         # Converts HTML to PDF
from django.template.loader import get_template  # Loads Django template
from django.http import HttpResponse            # Returns data to browser

# Converts HTML template + data into a downloadable PDF
def render_to_pdf(template_src, context_dict):
    template = get_template(template_src)                     # Load template
    html = template.render(context_dict)                      # Fill template with data
    result = io.BytesIO()                                     # Create in-memory buffer
    pdf = pisa.pisaDocument(io.BytesIO(html.encode("ISO-8859-1")), result)  # Convert HTML to PDF
    if not pdf.err:                                           # If no error
        return HttpResponse(result.getvalue(), content_type='application/pdf')  # Return PDF
    return                                                    # Else, return nothing

# View to generate and return discharge bill as PDF for a patient
def download_pdf_view(request, pk):
    dischargeDetails = models.PatientDischargeDetails.objects.filter(patientId=pk).order_by('-id')[:1]  # Get latest discharge record
    dict = {                                  # Prepare context data for template
    ...
    return render_to_pdf('hospital/download_bill.html', dict)  # Generate and return PDF
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            email = sub.cleaned_data['Email']  #cleaned_data is a dictionary containing the validated data from the form.
